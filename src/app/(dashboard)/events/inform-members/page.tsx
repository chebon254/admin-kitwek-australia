...
export default function EventInformMembersPage() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const response = await fetch("/api/events/inform-members");
  ...
  const handleSendClick = async () => {
    ...
    await fetchStatus();
    if (recipientCount === 0) {
      toast("No registered members found");
      return;
    }
  ...
  const fetchCampaignDetails = async (campaignId: string) => {
    try {
      const response = await fetch(`/api/events/inform-members?campaignId=${campaignId}`);
      if (!response.ok) return;
      const data = await response.json();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <a href="/events" className="text-gray-500 hover:text-gray-700">
          <ArrowLeft className="h-5 w-5" />
        </a>
        <div>
          <h1 className="text-2xl font-bold">Email All Members</h1>
          <p className="text-gray-600 mt-1">Send email and SMS notifications to all registered members</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 space-y-6">
        <div>
          ...
          <input
            ...
            placeholder="e.g. Upcoming Event - Kitwek Victoria Annual Gala 2026"
        ...
        <div>
          ...
          <textarea
            ...
            rows={12}
            placeholder={`We are excited to invite you to the Kitwek Victoria Annual Gala 2026!\n\nJoin us for an evening of celebration, networking, and community spirit. This is a memorable occasion to reconnect with fellow members and celebrate our achievements.\n\nDate: Saturday, 15 August 2026\nTime: 6:00 PM – 11:00 PM\nVenue: Crown Palladium, Melbourne\n\nTickets are available on the Kitwek Victoria website. We look forward to seeing you there!`}
            ...
            <div className="p-6 bg-white">
              <div style={{ maxWidth: 600, margin: "0 auto", fontFamily: "Arial, sans-serif", lineHeight: 1.6 }}>
                <div style={{ textAlign: "center", paddingBottom: 16, borderBottom: "2px solid #f0f0f0" }}>
                  ...
                  <p style={{ color: "#666" }}>Events &amp; Community Notice</p>
                </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>Send to All Members ({recipientCount})</span>
              </>
            )}
          </button>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4">
            ...
            <div className="p-6">
              ...
              <p className="text-sm text-gray-700">
                ...
                registered members? Members with email receive email, members with phone receive SMS, and members with both receive both.
              </p>
            </div>
